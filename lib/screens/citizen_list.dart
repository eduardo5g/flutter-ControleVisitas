...
class CitizenListState extends State<CitizenList> {
				...
				return Card(
					...
					child: ListTile(
						...
						leading: CircleAvatar(
							backgroundColor: getPriorityColor(this.citizenList[position].priority),
							child: getPriorityIcon(this.citizenList[position].priority),
						),

						title: Text(this.citizenList[position].name, style: nameStyle,),
  ...
  void updateListView() {
		...
		dbFuture.then((database) {
			...
			Future<List<Citizen>> citizenListFuture = citizenHelper.getCitizenList(homeIdent, 0);
